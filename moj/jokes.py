...
def view_joke(joke_id):
    ...
    if request.method == 'POST':
        ...
        if is_author:
            if action == 'update':
                ...
                db.execute('UPDATE joke SET body = ? WHERE id = ?', (new_body, joke_id))
                db.commit()
                ...
                return redirect(url_for('jokes.my'))  # ✅ correct, since you used endpoint='my'
            ...
            if existing_rating:
                db.execute('UPDATE rating SET score = ? WHERE id = ?', (score, existing_rating['id']))
                flash('Your rating has been updated!')
            else:
                db.execute('INSERT INTO rating (joke_id, user_id, score) VALUES (?, ?, ?)', (joke_id, g.user['id'], score))
    ...
    if not is_author:
        ...
        if not already_viewed:
            if g.user['joke_balance'] > 0:
                db.execute('INSERT INTO taken_jokes (user_id, joke_id) VALUES (?, ?)', (g.user['id'], joke_id))
